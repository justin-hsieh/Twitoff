...
def add_or_update_user(username):
    ...
    try:
        ...
        # Want as many recent non-retweet/reply statuses as we can get
        tweets = twitter_user.timeline(
            count=200, exclude_replies=True, include_rts=False,
            tweet_mode='extended', since_id=db_user.newest_tweet_id)

        # Check for new or recent tweets. If exists, retrieve tweet id
        if tweets:
            db_user.newest_tweet_id = tweets[0].id

        # Loop through tweets
        for tweet in tweets:
            # Calculate embedding on full tweet, then truncate for storing
